...
namespace ECSReact.Core
{
  ...
  public static class ECSActionDispatcher
  {
    ...
    /// <summary>
    /// DEPRECATED: Do not call from jobs directly.
    /// Instead, get the ParallelWriter before scheduling your job using GetJobCommandBuffer(),
    /// then use the extension method ECB.DispatchAction() for cleaner code.
    /// </summary>
    /// <param name="action">The action to dispatch</param>
    /// <param name="sortKey">Unique sort key for deterministic ordering</param>
    [Obsolete("Use ECB.DispatchAction() extension method instead. See ECBActionExtensions.")]
    public static void DispatchFromJob<T>(T action, int sortKey) where T : unmanaged, IGameAction
    {
      throw new InvalidOperationException(
        "DispatchFromJob should not be called directly from jobs. " +
        "Use the cleaner extension method approach instead:\n\n" +
        "Before scheduling your job:\n" +
        "  var ecb = ECSActionDispatcher.GetJobCommandBuffer();\n" +
        "  var handle = new MyJob { ECB = ecb }.ScheduleParallel();\n" +
        "  ECSActionDispatcher.RegisterJobHandle(handle);\n\n" +
        "In your job Execute method:\n" +
        "  ECB.DispatchAction(sortKey, action);");
    }
