...
namespace ECSReact.Samples.BattleSystem
{
  ...
  /// Action: ECSReact.Samples.BattleSystem.ReadyForNextTurn
  /// </summary>
  ...
  public partial struct NextTurnMiddleware_System : ISystem
  {
    ...
    public void OnCreate(ref SystemState state)
    {
      ...
      var queryBuilder = new EntityQueryBuilder(Allocator.Temp)
        .WithAll<ECSReact.Samples.BattleSystem.ReadyForNextTurn, ActionTag>();
      actionQuery = state.GetEntityQuery(queryBuilder);
      queryBuilder.Dispose();

      state.RequireForUpdate(actionQuery);
      state.RequireForUpdate<ECSReact.Samples.BattleSystem.ReadyForNextTurn>();
    }
    ...
    private void ProcessMiddleware(ref SystemState state)
    {
      ...
      var type = new ComponentType(typeof(ECSReact.Samples.BattleSystem.ReadyForNextTurn));
      bool isZeroSized = type.IsZeroSized;

      if (isZeroSized)
      {
        // Zero-sized: no data to read, use default
        var action = default(ECSReact.Samples.BattleSystem.ReadyForNextTurn);
      ...
      else
      {
        ...
        var actions = actionQuery.ToComponentDataArray<ECSReact.Samples.BattleSystem.ReadyForNextTurn>(Allocator.Temp);
